...
| 變數名稱 | 用途 |
|---|---|
| `NOTION_API_KEY` | Notion API 金鑰（在 Vercel env 設定，勿提交） |
| `NOTION_IMPORT_STATUS_DB` | 進口批次主表 DB ID（在 Vercel env 設定） |
| `NOTION_SHIPMENT_RECORDS_DB` | 出貨紀錄 DB ID（在 Vercel env 設定） |
| `NOTION_CHANGE_LOG_DB` | 修改紀錄 DB ID（在 Vercel env 設定） |
| `NOTION_DEMAND_DB` | 需求清單 DB ID（在 Vercel env 設定） |
| `LINE_CHANNEL_SECRET` | LINE Messaging API 的 Channel secret（驗證 webhook 簽章用） |
| `GOOGLE_SERVICE_ACCOUNT_JSON` | GCP 服務帳號 JSON（完整內容，單行） |
| `GOOGLE_DRIVE_FOLDER_ID` | Shared Drive ID（在 Vercel env 設定） |
| `GMAIL_USER` | 寄件帳號（在 Vercel env 設定） |
| `GMAIL_APP_PASSWORD` | Gmail App Password（在 Vercel env 設定，**勿提交實際值**） |
| `EDIT_PASSWORD` | 編輯密碼（**請設高強度密碼，勿用預設或弱密碼**） |
| `LOGISTICS_PASSWORD` | 物流業者後台密碼（**必須設定**，未設時 `/api/portal-auth` 會 fail-closed 拒絕） |
| `AUTH_SECRET` | Auth cookie HMAC secret（建議獨立設定；未設時 fallback 用 EDIT_PASSWORD） |
...
### 需求清單（DEMAND_DB）
欄位：`項目名稱`(title)、`門市`(select，15間，含3間即將開幕)、`商品`(rich_text)、`數量`(rich_text)、`日期`(date)、`狀態`(select：待確認/待處理/已安排/已完成)、`來源`(select：LINE/手動)、`備註`(rich_text)、`原始訊息`(rich_text)、`LINE訊息ID`(rich_text，去重用)

> 來源：LINE群組訊息經 `/api/line-webhook` 自動解析寫入（狀態=待確認），或在 `/demand` 頁面手動新增/貼上文字解析（狀態=待處理）
...
app/
  ...
  layout.tsx
  demand/page.tsx           # 需求清單頁面（自動抓取/手動新增/狀態切換/排序）
  api/
    ...
    demand/route.ts          # GET 需求清單 / POST 新增需求
    demand/[id]/route.ts     # PATCH/DELETE 需求
    line-webhook/route.ts    # POST LINE訊息接收端（簽章驗證+自動解析）
...
lib/
  ...
  parseDemandText.ts        # 需求清單解析（門市/商品/數量/日期），LINE webhook與demand頁共用
```
...
**營業中（open）12 間**：LaLaport台中、桃園春日、新北中和環球、新莊宏匯、高雄漢神巨蛋、南港LaLaport、IKEA台中南屯、高雄夢時代、台南小北門、台南三井Outlet（= MOP）、台中漢神中港、台北美麗華（大直，2026-07-06 開幕）
