...
### 需求清單（DEMAND_DB）
欄位：`項目名稱`(title)、`門市`(select，14間，含3間即將開幕)、`商品`(rich_text)、`數量`(rich_text)、`日期`(date)、`狀態`(select：待確認/待處理/已安排/已完成)、`來源`(select：LINE/手動)、`備註`(rich_text)、`原始訊息`(rich_text)、`LINE訊息ID`(rich_text，去重用)
